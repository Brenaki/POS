...
def generate_bags(X_train: np.ndarray, y_train: np.ndarray, nr_bags: int, tam_bags: float) -> dict:
    ...
    for _ in range(nr_bags):
        _, _, _, _, id_bag, _ = train_test_split(
            X_train, y_train, indices, test_size=tam_bags, stratify=y_train
        )
