...
def build_pool_rf(X_train, y_train, M: int, random_state: int):
...
def per_dataset_summary(summary_df) -> list[dict]:
    ...
    for (ds, mode), grp in summary_df.groupby(["dataset", "mode"]):
        rows.append({
            ...
            "oracle_1_mean": grp["oracle_1"].mean(), "oracle_1_std": grp["oracle_1"].std(),
            "oracle_M_mean": grp["oracle_M"].mean(), "oracle_M_std": grp["oracle_M"].std(),
...
def build_summary_row(ds_name, fold_idx, mode, metrics, n_test) -> dict:
    ...
    return {
        ...
        "oracle_1": curve[0] if curve else None,
